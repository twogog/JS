window.addEventListener('DOMContentLoaded', function() {
    ...
    //Форма отправки без JSON и с ним  через Promise       input в форме должен содержать name!!!!!
    ...
    function sendForm(elem) {
        elem.addEventListener('submit', function(event) {
            ...
            let formData = new FormData(elem);                 //Все, что ответил пользователь в форме
    ...
    function showSlides(n) {
        if (n > slides.length) {            //листаем вперед
            slideIndex = 1;
        }

        if (n < 1) {                        //листаем назад
    ...
    let persons = document.querySelectorAll('.counter-block-input')[0],
        ...
        allinputs = document.querySelectorAll('.counter-block-input'),
    ...
    function checkInputs() {
        allinputs.forEach(function(a, b) {
            if (allinputs[b].value == '' || allinputs[b].value == 0){
                totalValue.innerHTML = 0;
            }
        });
    }
